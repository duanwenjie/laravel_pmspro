...
use App\Models\Pms\Purchaseplan;
...
class PmsService
{
    ...
    protected const noPoStatus = [10,70,90,100]; // 不需要统计的采购单明细状态
     ...
     * @author  dwj
     */
    public static function getNoOrderPrNum()
    {
     ...
     * @author dwj
     */
    public static function getNoPrintNum()
    {
        $times = Carbon::now()->format('Y-m-d H:i:s');
    ...
    public static function getPurchaseOnWayNum2()
    {
        ...
        $i = 0;
        $date = Carbon::now()->subDays(60)->format('Y-m-d'); // 取最近60天的数据
     ...
     * @author dwj
     */
    public static function initOmsStockNum()
    {
    ...
    /**
     * 同时更新PMS未建单PR数、PMS已建单未打印数、PMS采购在途
     * @todo先看看效果，后期稳定后将sql转成代码
     */
    public static function updatePmsQty(){
        $data = self::getPmsSkuQtys();
        $data = array_chunk($data,3000);
        foreach ($data as $temp) {
            $sql = Formater::sqlInsertAll('mrp_base_sku_stock_lists', $temp, ['no_order_pr_num','no_print_num','purchase_on_way_num']);
            $sql && DB::insert($sql);
        }
    }

    public static function getPmsSkuQtys(){
        $build = Purchaseplan::query()
            ->whereIn('state', array_keys(self::prNoUse))
            ->select([
                'sku',
                DB::raw('warehouse_id as warehouseid'),
                DB::raw("sum(surplus_quantity) as no_order_pr_num"),
                DB::raw("0 as no_print_num"),
                DB::raw("0 as purchase_on_way_num"),
            ])
            ->groupBy('sku', 'warehouse_id');
        $tmp = DB::connection('pms')->table('purchaseorder_detail')
            ->whereIn('state', array_keys(self::poNoPrint))
            ->select([
                'sku',
                DB::raw('warehouse_id as warehouseid'),
                DB::raw("0 as no_order_pr_num"),
                DB::raw("sum(quantity) as no_print_num"),
                DB::raw("0 as purchase_on_way_num"),
            ])
            ->groupBy('sku', 'warehouse_id');
        $build = $build->unionAll($tmp);
        $tmp = DB::connection('pms')->table('purchaseorder_detail as a')
            ->join('purchaseorder as b', 'b.id', '=', 'a.purchaseorder_id')
            ->whereIn('a.state', array_keys(self::poOnWay))
            ->whereRaw('b.create_time >= DATE_SUB(curdate(),INTERVAL 60 DAY)')
            ->select([
            ...
            ->groupBy('a.sku', 'a.warehouse_id');
        $build = $build->unionAll($tmp);
        $data = DB::connection('pms')->table(DB::raw("({$build->toSql()}) as t"))
            ->mergeBindings($build->getQuery())
            ->select([
                'sku',
                'warehouseid',
                DB::raw("sum(no_order_pr_num) no_order_pr_num"),
                DB::raw("sum(no_print_num) no_print_num"),
                DB::raw("sum(purchase_on_way_num) purchase_on_way_num"),
            ])
            ->groupBy('sku','warehouseid')->get()->map(function($item){
                return (array)$item;
            });
        return current($data);
    }
